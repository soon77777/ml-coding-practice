...
def main():
    ...
    with open('%s_naver_%s.json' % (srcTeext, node), 'w', encoding = 'utf8') as outfile:
    ...
    print('%s_naver_%s.josn SAVED' % (srcText, node))
...
def getRequestUrl(url):
    ...
    req.add_header("X-NaverClient-Secret", client_secret)

    try:
        response = urllib.request.urlopen(req)
        if response.getcode() == 200:
            print("[%s] Url Requeest Success & datetime.datetime.now()")
